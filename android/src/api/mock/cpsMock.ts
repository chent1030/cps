...
export const getCpsMockResponse = <T>({ method, path, params, body }: MockRequest) => {
  ...
  if (method === 'GET' && path === '/api/cps/master/lines')
    return (lines[`${text(params?.factory)}|${text(params?.area)}`] ?? []) as T
  ...
  if (method === 'GET' && path === '/api/cps/master/categories')
    return (categories[num(params?.parentId)] ?? categories[0]) as T
