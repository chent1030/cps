...
const send = async <T>(path: string, options: RequestOptions = {}) => {
  ...
  try {
    ...
    return fallbackOrThrow<T>(method, path, options.params, rawBody, error instanceof Error ? error.message : 'CPS API request failed')
  }
}
...
const sendWithFetch = async <T>(path: string, options: RequestOptions) => {
  ...
  if (!response.ok) {
    return fallbackOrThrow<T>(method, path, options.params, options.body, `CPS API ${response.status}: ${response.statusText}`)
  }
...
const sendWithUni = <T>(path: string, options: RequestOptions) => {
  ...
  return new Promise((resolve: (value: T) => void, reject) => {
    uni.request({
      ...
      fail(error) {
        try {
          resolve(fallbackOrThrow<T>(method, path, options.params, options.body, error.errMsg || 'CPS API request failed'))
        } catch (fallbackError) {
