...
export function PageHead({ title, detail, children }: { title: string; detail: ReactNode; children?: ReactNode }) { return <div className="page-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 16, flexWrap: 'wrap' }}><div><Typography.Title heading={4}>{title}</Typography.Title><Typography.Paragraph className="page-description">{detail}</Typography.Paragraph></div>{children}</div> }
